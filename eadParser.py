...
def parseEAD(xmlfile):
    with open(xmlfile, "rb") as xmlrbfile:
        parse = xmltodict.parse(
            xmlrbfile, force_list={"note", "c"}, dict_constructor=dict
        )
        ead = parse["ead"]
            ...
            if k not in ["head", "@level", "did"]:
                for subelement in serie[k]:
                    if type(subelement) != str:
                        children.append(
                            parseDsc(subelement, parentCollection=parentCollection)
                        )
...
def parseCollection(ead):
    ...
    collection.children = [
        parseDsc(serie, parentCollection=collection) for serie in archdesc["dsc"]["c"]
    ]
...
if __name__ == "__main__":
    ...
    for f in os.listdir("/home/leon/Documents/Golden_Agents/saaA2A/data/ead/"):
        ...
        print(f)
        ead = parseEAD(
            os.path.join("/home/leon/Documents/Golden_Agents/saaA2A/data/ead/", f)
        )
