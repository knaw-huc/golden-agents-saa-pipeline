...
def main(datafile, path, gz=True):
    ...
    for n, d in enumerate(data, 1):
        ...
        groomRole = Bruidegom(
            ...
            carriedBy=[groom],
            label=[Literal(f"{pnGroom.label} in de rol van Bruidegom", lang="nl")],
        )

        # status
        if maritalStatusName := d["groom"]["marital_status_modern"]:
            status, statusName = thesaurus(
                maritalStatusName, Status, graph, thesaurusGraph
            )
        ...
        if d["groom"]["origin"]:
            ...
            if ecarticoPlace := id2ecartico_places_groom.get(locURIstr):
                ...
                place = SchemaPlace(
                    URIRef(ecarticoPlace["@id"]), label=[ecarticoPlace["label"]]
                )
        ...
        if d["groom"]["homeLocation"]:
            ...
            location = LocationObservation(
                URIRef(d["groom"]["id"] + "-address"), label=[locName]
            )
        ...
        if d["groom"]["occupation"]:
            ...
            occupation = OccupationObservation(
                URIRef(d["groom"]["id"] + "-occupation"), label=[occName]
            )
            ...
            # Reconstruction
            if d["groom"]["occupation_modern"] or d["groom"]["occupation_hisco"]:
                sdoOcc = SchemaOccupation(
                    unique(
                        d["groom"]["occupation_modern"],
                        d["groom"]["occupation_hisco"],
                        "schema",
                    ),
                    label=[d["groom"]["occupation_modern"] or d["groom"]["occupation"]],
                )
                ...
                reconstruction = OccupationReconstruction(
                    unique(
                        d["groom"]["occupation_modern"], d["groom"]["occupation_hisco"]
                    ),
                    wasDerivedFrom=[occupation],
                    sameAs=[sdoOcc],
                    prefLabel=[
                        d["groom"]["occupation_modern"] or d["groom"]["occupation"]
                    ],
                )
        ...
        if d["groom"]["exWife"]:
            ...
            exWifeRole = EarlierWife(
                ...
                carriedBy=[exWife],
                label=[
                    Literal(f"{pnExWife.label} in de rol van Eerdere Vrouw", lang="nl")
                ],
            )
        ...
        for w in d["groom"]["witnesses"]:
            ...
            witnessRole = Getuige(
                ...
                carriedIn=registrationEvent,
                carriedBy=[witness],
                label=[Literal(f"{pnWitness.label} in de rol van Getuige", lang="nl")],
            )

            if w["relation"]:

                relation = Relation(
                    URIRef(w["id"] + "-relation"), label=[w["relation"]]
                )
                ...
                relationRole = RelationRole(
                    ...
                    label=[Literal(f"{w['relation']} ({pnWitness.label})", lang="nl")],
                )
        ...
        if d["bride"]["origin"]:
            ...
            location = LocationObservation(
                URIRef(d["bride"]["id"] + "-origin"), label=[locName]
            )
        ...
        if d["bride"]["homeLocation"]:
            ...
            location = LocationObservation(
                URIRef(d["bride"]["id"] + "-address"), label=[locName]
            )
        ...
        if d["bride"]["occupation"]:
            ...
            occupation = OccupationObservation(
                URIRef(d["bride"]["id"] + "-occupation"), label=[occName]
            )
            ...
            # Reconstruction
            if d["bride"]["occupation_modern"] or d["bride"]["occupation_hisco"]:
                sdoOcc = SchemaOccupation(
                    unique(
                    ...
                    label=[d["bride"]["occupation_modern"] or d["bride"]["occupation"]],
                )
                ...
                reconstruction = OccupationReconstruction(
                    unique(
                        d["bride"]["occupation_modern"], d["bride"]["occupation_hisco"]
                    ),
                    wasDerivedFrom=[occupation],
                    sameAs=[sdoOcc],
                    prefLabel=[
                        d["bride"]["occupation_modern"] or d["bride"]["occupation"]
                    ],
                )
        ...
        if d["bride"]["exHusband"]:
            ...
            pnExHusband = PersonName(
                ...
                surnamePrefix=d["bride"]["exHusband"]["hasName"]["surnamePrefix"],
            ...
            exHusbandRole = EarlierHusband(
                ...
                carriedBy=[exHusband],
                label=[
                    Literal(f"{pnExHusband.label} in de rol van Eerdere Man", lang="nl")
                ],
            )
